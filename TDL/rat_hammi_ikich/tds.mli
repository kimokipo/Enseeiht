...
type info_ast = info ref and info =
  ...
  son type, le nom du variable struct auquel est associé, et son adresse ie son déplacement (int) par rapport à un registre (string) *)
  | InfoVar of string * typ *string* int * string
  (* Information associée à un type nommé : son nom et son vrai type*)
  | InfoType of string * typ
  (* Information associée à un type Struct : son nom, la liste des champs avec leur types et info_ast, son adresse et son déplacement *)
...
(* Modifie le nom de variable Struct auquel est associé *)
val modifier_struct_var_info : string -> info_ast -> unit
