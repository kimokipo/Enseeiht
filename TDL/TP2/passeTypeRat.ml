...
let rec analyse_type_expression e = (match e with 
  ...
  begin(match info_ast_to_info info_ast with
              | InfoVar (_,t,_,_) -> Ast.AstType.Ident info_ast,t
              | InfoConst (_,n) -> Ast.AstType.Entier n,Int
              | _ -> assert false)
  end
 ...
 begin (match info_ast_to_info info with
            ...
            (try 
              let nes,tes = List.split(List.map2 (fun e t -> 
                let ne,te = analyse_type_expression e in if (est_compatible te t) then (ne,te) else 
...
let rec analyse_type_instruction i =
  ...
  | AstTds.Declaration (t, info, e) ->    
            ...
            let ne,te = analyse_type_expression e in if (est_compatible te t) then
  ...
  | AstTds.Affectation (info,e) ->
          ...
          begin
            ...
            | InfoVar (_,t,_,_) -> 
              ...
              if (est_compatible te t) then Ast.AstType.Affectation (info, ne),None else raise (TypeInattendu (te,t))
...
let analyse_type_fonction (Ast.AstTds.Fonction(t,info,lp,li)) = 
        List.iter (fun (x,y) -> modifier_type_info x y) lp;
