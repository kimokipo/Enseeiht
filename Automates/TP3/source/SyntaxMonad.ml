...
and parseEX stream =
  ...
  (match (peekAtFirstToken stream) with
    ...
    (print_endline "= ER EX");
        ...
        parseEX
    (* regle #7 *)
    | InToken | ThenToken | ElseToken |RightParenthesisToken -> inject stream
...
and parseTX stream =
  ...
  (match (peekAtFirstToken stream) with
    ...
    (print_endline "- T TX");
        ...
        parseTX
    (* regle 11 *)
    | InToken | ThenToken | ElseToken | RightParenthesisToken | EqualToken -> inject stream
    | EOSToken -> inject stream
    | _ -> Failure)
...
and parseFX stream =
  ...
  (match (peekAtFirstToken stream) with
    ...
    (print_endline "/ F FX");
        ...
        parseFX
    (* regle 15 *)
    | (InToken | ThenToken | ElseToken |RightParenthesisToken | EqualToken | PlusToken | MinusToken) ->
        inject stream
    | EOSToken -> inject stream
    | _ -> Failure)
...
and parseARG stream =
  ...
  (match (peekAtFirstToken stream) with
    ...
    accept RightParenthesisToken
    (* regle 26 *)
    | ( ThenToken | ElseToken | InToken | RightParenthesisToken | DivideToken |TimesToken | EqualToken | PlusToken |MinusToken) ->
    inject stream
    | EOSToken -> inject stream
    | _ -> Failure)
;;
