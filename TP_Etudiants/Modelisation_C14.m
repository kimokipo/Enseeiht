...
for i = 1:nb_iterations_max
	...
	options(3) = i;
	[beta, norm_grad_f_beta, f_beta, norm_delta, k, exitflag] = Algo_Gauss_Newton(@(beta) residu_C14(beta, Donnees),@(beta) J_residu_C14(beta, Donnees),beta0, options);
...
for i = 1:nb_iterations_max
    ...
    [beta, norm_grad_f_beta, f_beta, norm_delta, k, exitflag] = Algo_Newton(@(beta) Hess_f_C14(beta, Donnees,@(beta) residu_C14(beta, Donnees),@(beta) J_residu_C14(beta, Donnees)),beta0, options);
    % Affichage des valeurs
    disp([k beta(1) beta(2) norm_grad_f_beta f_beta norm_delta exitflag])
    % Visualisation
    A = beta(1)*exp(-beta(2)*T);
...
%
    % TO DO %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    res = -beta(1)*exp(-beta(2)*donnees(:,1))+donnees(:,2);
...
%
    ...
    n=size(donnees,1);
    S=zeros(2*n,2);
    S(1:2:end,1)= zeros(n,1);
    S(2:2:end,1)= donnees(:,1).*exp(-beta(2)*donnees(:,1));
    S(1:2:end,2)= donnees(:,1).*exp(-beta(2)*donnees(:,1));
    S(2:2:end,2)= -(donnees(:,1).*donnees(:,1)).*(beta(1)*exp(-beta(2)*donnees(:,1)));
    H=zeros(2,2);
    for i = 1:n
        H = H + res(i)*S((2*i-1):(2*i),:);
    end 
    H_f = H +(transpose(J_res)*J_res);
