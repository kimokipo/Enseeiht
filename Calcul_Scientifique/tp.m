...
for i = 1:k
  ...
  po(2,i) = norm(eye(n)-Qmgs'*Qmgs);
    ...
    loglog(x,po(1,:),'g','lineWidth',2)
    hold on
    loglog(x,po(2,:),'y','lineWidth',2)
    grid on
    leg = legend('Gram-Schmidt classique',...
                 'Gram-Schmidt modifie',...
                 'Gram-Schmidt classique itérée',...
                 'Gram-Schmidt modifie itérée',...
             'Location','NorthWest');
    set(leg,'FontSize',14);
    xlim([x(1) x(end)])
    hx = xlabel('\textbf{Conditionnement $\mathbf{\kappa(A_k)}$}',...
...
for i = 1:k
  ...
  po(2,i) = norm(eye(n)-Qmgs'*Qmgs);
    ...
    loglog(x,po(1,:),'o','lineWidth',2)
    hold on
    loglog(x,po(2,:),'p','lineWidth',2)
    grid on
    leg = legend('Gram-Schmidt classique',...
                 'Gram-Schmidt modifie',...
                 'Gram-Schmidt classique itérée',...
                 'Gram-Schmidt modifie itérée',...
                 'Gram-Schmidt classique croisée',...
                 'Gram-Schmidt modifie croisée',...
             'Location','NorthWest');
    set(leg,'FontSize',14);
    xlim([x(1) x(end)])
    hx = xlabel('\textbf{Conditionnement $\mathbf{\kappa(A_k)}$}',...
