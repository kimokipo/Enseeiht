...
subroutine facto(m, ldm, n, ipivot, p, q, ierr)
  ...
  double precision :: eps,norme_f_n
  norme_f_n = norme_f(m,n,n)
  ...
  do k = 1,(n-1) 
	if abs(m(k,k)) < (eps*norme_f_n)
		return
	end if
	m(k+1:n,k) = m(k+1:n,k) / m(k,k)
	m(k+1:n,k+1:n) = m(k+1:n,k+1:n) - matmul(m(k+1:n,k),m(k,k+1:n))
	call maj_det(mantisse_det,exponent_det,m(k,k))
  end do
  p(n) = n
  call maj_det(mantisse_det,exponent_det,m(n,n))
  !!!!!!!!!!!!!!!!! A ECRIRE !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! 
...
!*********************************************
double precision function majDet(m)
...
subroutine analyse_erreur(m,ldm,n,b,x,r,xexact)
  ...
  write(*,*) "le residu équilibré est :"
  write(*,*) norme(r,n)/(norme_f(m,n,n)*norme(x,n)+norme(b,n))

  write(*,*) "l'erreur relative est :"
  write(*,*) erreur
