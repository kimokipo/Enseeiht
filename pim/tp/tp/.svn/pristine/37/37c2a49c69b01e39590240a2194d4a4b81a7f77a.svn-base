...
void create(string *chaine_dest, char *chaine_src){
    int longueur=strlen(chaine_src);
    chaine_dest->str=malloc((longueur+1)*sizeof(char));
    for (int i=0;i<longueur;i++){
        chaine_dest->str[i]=chaine_src[i];
    }
    chaine_dest->str[longueur]='\0';
    chaine_dest->N=longueur+1;
 ...
 * (attention à la precondition).Precondition : i<chaine->N & 0<=i ;
*/
void delete(string *chaine, int i){
    for (int j=i;j<chaine->N-1;j++){
        chaine->str[j]=chaine->str[j+1];
    }
    chaine->str=realloc(chaine->str,sizeof(chaine->str)-sizeof(char));
    chaine->N=chaine->N-1;
}
