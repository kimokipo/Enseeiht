...
void prefix_inc_par(int l, int *pref){
  ...
  int position_debut,chunk,i,iam,nth;

/* ===================================================================== 
   Write the parallel inclusive prefix scan algorithm here 
   ===================================================================== */
  #pragma omp parallel private(i,iam,nth,position_debut,chunk)
  
    iam = omp_get_thread_num();
    nth = omp_get_num_threads();
  chunk = l/nth;

  position_debut = chunk * iam;


  prefix_inc_seq(chunk,pref+position_debut);
  
  loc_last[iam] = pref[position_debut+chunk-1];
...
#pragma omp barrier
  ...
  for(i=position_debut;i<(position_debut+chunk);i++){
    pref[i] += loc_last[iam];
  } 
