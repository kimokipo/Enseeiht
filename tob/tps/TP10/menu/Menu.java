...
public class Menu  {
	...
	private List<String> raccourcis;
	...
	public Menu(String sonTitre) {
		...
		this.raccourcis = new ArrayList<String>();
		this.raccourcis.add("nbv");
		this.titre = sonTitre;
		this.selection = null;
	...
	public /*@ pure @*/ void afficher() {
		...
		for (int i = 1; i <= this.getNbEntrees(); i++) {
			this.getEntree(i).afficher(i,this.raccourcis.get(i));

		}

		// Afficher la possibilité de quitter
		this.getEntree(0).afficher(0,this.raccourcis.get(0));
	...
	public void selectionner() {
		...
		boolean raccourci_valide = false;
		do {
			String raccourci = Console.readLine("Votre choix : "); 
			choix = this.raccourcis.indexOf(raccourci);
			try{
				if (choix == -1) {
					choix = Integer.parseInt(raccourci);
				}
				raccourci_valide = true;
			}
			...
			if(!raccourci_valide){
				System.out.println("Le raccourci n'est pas valide ");
			}
			else if (!choix_valide) {
