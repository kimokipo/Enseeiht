...
public class PointDeVente {
	...
	public int getPrix(Carburant carburant, LocalDateTime date) {
		NavigableMap<LocalDateTime, Integer> lePrix = (this.getPrix().containsKey(carburant)) ? this.getPrix().get(carburant) : null;
		return (lePrix != null & lePrix.containsKey(date)) ? lePrix.get(date) : 0;
