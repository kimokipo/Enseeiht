...
let rec fusionne ordre l1 l2 = 
   match (l1,l2) with  
    |([],_) -> l2
    |(_,[]) -> l1
    |(t1::q1,t2::q2) -> if (ordre t1 t2) then t1::(fusionne ordre q1 l2) else t2::(fusionne ordre q2 l1)
...
let temps f l =
    let t = Sys.time() in
    let fl = f l in
    Printf.printf "Execution time of f: %fs\n" (Sys.time() -. t);
     fl

(* CONTRAT
Fonction qui décompose une liste en deux listes de tailles égales à plus ou moins un élément avec récursivité terminale
Paramètre : l, la liste à couper en deux
Retour : deux listes
*)

let scinde2 l = List.fold_right (fun e (l1,l2) -> if (List.length l1) >= (List.length l2) then (l1,e::l2) else (e::l1,l2)) l ([],[])

let scindeTerminale l =  
  let n = List.length l in
  let rec scindeQueue (l1,l2) i = 
    if i = 0 then (l1,l2) else
    scindeQueue (List.tl l1,(List.hd l1)::l2) (i-1) in
  scindeQueue (l,[]) (n/2)

let fusioneeTerminale ordre l1 l2 = 
  let rec fusionneQueue (l1,l2,l) i =
    if i = 0 then l else
    if l1 = [] then l@l2 else 
    if l2 = [] then l@l1 else  
    if (ordre (List.hd l1) (List.hd l2)) then fusionneQueue (List.tl l1,l2,l@[(List.hd l1)]) (i-1) else 
      fusionneQueue (l1,List.tl l2,l@[(List.hd l2)]) (i-1)
  in fusionneQueue (l1,l2,[]) ((List.length l1)+(List.length l2))

(*let rec tri_fusion2 ordre l =
  let n = List.length l in
    let rec triQueue (l1,l2) i =  
     if i = 0 then (fusioneeTerminale ordre l1 l2) else
     let (q1,q2) = scindeTerminale l in
      (tri_fusion ordre l1) (tri_fusion ordre l2)*)
 
let rec garder l n = 
  match l with
  |[] -> l
  |t::q -> if n=0 then [] else t::(garder q (n-1))
