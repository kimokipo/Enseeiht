...
(*   paramètres : - une liste d'éléments (caractères dans le cas d'un dico)   *)
(*                - un arbre n-aire                                           *)
(*   résultat   : l'arbre n-aire avec l'élément retiré                                 *)
(******************************************************************************)
...
(******************************************************************************)
(*   fonction parcours d'un arbre                      *)
(*   signature  : parcours_arbre : 'a arbre -> 'a list list                    *)
(*   paramètres : - un arbre n-aire                                            *)
(*   résultat   : la liste des élément dans l'ordre lexicographique de l'arbre *)
(******************************************************************************)
let rec parcours_arbre (Noeud (b, lb)) =
  match (b,lb) with
  | (true,[]) -> [[]]
  | (false,[]) -> []
  | (b,(t,a)::q) -> if b = true then ([]::(List.map (fun x -> t::x) (parcours_arbre a)) @ (parcours_arbre (Noeud (false, q)))) else 
            (List.map (fun x -> t::x) (parcours_arbre a) @ (parcours_arbre (Noeud (false, q))))


(******************************************************************************)
(*   fonction normalisation d'un arbre                      *)
(*   signature  : normalisation_arbre : 'a arbre -> 'a arbre                  *)
(*   paramètres : - un arbre n-aire                                           *)
(*   résultat   : l'arbre n-aire avec les branches inutiles éliminées         *)
(******************************************************************************)
let rec normalisation_arbre (Noeud (b, lb)) =
  match (b,lb) with
  | (_,[]) -> (Noeud (b, lb))
  | (b,(t,a)::q) -> let (Noeud (_,nouvelle_q)) = normalisation_arbre (Noeud (b, q))
                     in let nouvelle_arbre = (normalisation_arbre a) 
                     in (Noeud (b, maj t nouvelle_arbre ((t,a)::nouvelle_q)))
