...
let rec height tr = match tr with 
    | (Leaf _) -> 0
    | Node (a,b) -> let h1 = (height a) in let h2 = (height b) in if h1 > h2 then 1+h1 else 1+h2
...
let rec num_nodes tr = match tr with 
    | (Leaf _) -> 1
    | Node (a,b) -> let n1 = (num_nodes a) in let n2 = (num_nodes b) in 1+n1+n2
...
let rec num_values tr =  match tr with 
    ...
    | Node (a,b) -> let v1 = (num_values a) in let v2 = (num_values b) in v1+v2
 ...
 *)
let rec bits n addr = if n = 0 then [] else (addr mod 2) :: (bits (n-1) (addr / 2))
 ...
 *)
let rec search tr addrl = if (height tr ) > (List.length addrl) then failwith "taille de l'addresse n'est pas supérieure à la profondeur de l'arbre" else match (tr,addrl) with 
     | (Leaf c, _) -> c 
     | (Node (_,_) , []) -> _0
     | (Node (a,b) , t::q) -> if t = 0 then (search a q) else (search b q)
...
let rec update tr x addrl = match (tr,addrl) with 
   | (Leaf _, []) -> Leaf x
   | (Leaf c, (t::q)) -> if t = 0 then Node((sprout x q),Leaf c) else Node(Leaf c,(sprout x q))
   | (Node (_,_) , []) -> failwith "taille de l'addresse n'est pas supérieure à la profondeur de l'arbre"
   | (Node (a,b) , t::q) -> if t = 0 then Node ((update a x q),b) else Node (a,(update b x q))
