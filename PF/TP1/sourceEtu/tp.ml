...
(** 
   ...
   (x1, y1) et (x2, y2)
*) 
let coeff_directeur (x1,y1) (x2,y2) = if x1 = x2 then failwith " droite verticale" else (y2-y1)/(x1-x2)
...
let rec padovan n = if n = 0 || n = 1 then  0  else if n = 2 then 1 else
...
let estPremier n = if n = 0 || n = 1 then false else if n = 2 then true else 
               let rec divisible k p =
                  if k mod p = 0 then true else if p = 2 then false else divisible k (p-1)
               in not (divisible n (n-1));;
...
*)
let rec dragon (xa,ya) (xb,yb) n = if n = 0 then
   dessine_segment (xa,ya) (xb,yb) else let (xc,yc) = ((xa+xb)/2 + (yb-ya)/ 2 , (ya+yb)/2 + (xa-xb)/2 ) in
      List.iter (fun (x,y,z) -> dragon x y z) [((xa,ya),(xc,yc),(n-1));((xc,yc),(xb,yb),(n-1))]
