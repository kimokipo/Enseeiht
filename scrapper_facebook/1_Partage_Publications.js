...
import message from './message.js'
 ...
 (async function () {
    ...
    if (process.argv.length < 6) {
        ...
        if (nom && (await getIdAmbassadeur(nom)) && prenom && (await getLoginAmbassadeur(prenom)) && id_campagne && id_segment && message) {
	            ...
	            const Ids = await posterGroupes(page,id_ambassadeur,id_campagne,id_segment,message,rapport)
	            
	            var cookies = await page.cookies();
                await UpdateCookiesAmbassadeur(id_ambassadeur,JSON.stringify(cookies, null, 2))
	            var message_fini = 'Script Fini'
	            await page.evaluate(async message => {
...
function AfficherMenu(){
            ...
            console.log('node 1_Partage_Publications.js Prenom nom id_campagne id_segment \n')
