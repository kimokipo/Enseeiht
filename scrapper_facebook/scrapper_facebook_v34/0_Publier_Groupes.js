/* Publier automatiquement par l'ambassadeur (prenom) un post de la campagne (id_campagne), 
...
import publierGroupes from './src/publierGroupes.js'
...
import login from './src/login.js'
 ...
 (async function () {
    ...
    const semi_legende = process.argv.slice(6).join(' ')
    
    var groupLink_probleme = ''
    ...
    process.on('unhandledRejection', async (reason, promise) => {
            ...
            if (promise.indexOf('Error: Protocol error (Runtime.callFunctionOn): Execution context was destroyed.') != -1){
	            ...
	            const Ids = await publierGroupes(page,id_ambassadeur,id_campagne,id_segment,semi_legende,groupLink_probleme,rapport)
    ...
    if (process.argv.length < 6) {
        ...
        if (nom && (await getIdAmbassadeur(prenom,nom)) && prenom && (await getLoginAmbassadeur(prenom,nom))  && id_campagne != null && id_segment) {
...
function AfficherMenu(){
            ...
            console.log('node 0_Publier_Groupes.js Prenom nom id_campagne id_segment lien/contenu_legende_post_partage\n')
