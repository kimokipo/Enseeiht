...
dk_ask= (2*bi2de(reshape(Bits,2,length(Bits)/2).')-3).';
dk_ask = reshape(dk_ask,length(dk_ask),1);

dirac=zeros(Ns*N_bits/2,1);

dirac(1:Ns:end,1) = dk_ask;


figure;
plot(real(dk_ask),imag(dk_ask));
...
ylabel('signal g généré sans canal par la Chaine_4-ASK ');
...
for i = 1:101
    ...
    Bits_renconstruits = reshape(de2bi((symboles+ 3)/2).',length(Bits),1);
...
ylabel('Taux  d’erreur  binaire TEB pour la Chaine_4-ASK');
hold on
vecteur_Teb_theorique = 2*(1-1/M)*qfunc(sqrt(6*log2(M)*10.^(vecteur_db_Eb_N0/10)/(M.^2-1)))/log2(M);
semilogy(vecteur_db_Eb_N0,vecteur_Teb_theorique);

legend('Taux  d’erreur  binaire TEB simulé pour la Chaine_4-ASK ','Taux  d’erreur  binaire TEB théorique pour la Chaine_4-ASK');
title('Comparaison entre Taux  d’erreur  binaire TEB simulé et théorique pour la Chaine_4-ASK');
