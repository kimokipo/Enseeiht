%Parametres de la chaine_16-QAM.
close all;
...
dk_qam = qammod(Bits,8);

dirac=zeros(Ns*N_bits,1);
dirac(1:Ns:end,1) = dk_qam;
...
ylabel('signal g généré sans canal par la chaine_16-QAM. ');
...
for i = 1:101
    ...
    %xlabel('Temps en secondes');
    %ylabel('Diagramme de loeil sans canal de la chaine chaine_16-QAM ');

    n0 = Ns;
    z_hantillonne = z(n0:Ns:end);
    
    Bits_renconstruits = qamdemod(z_hantillonne,8);
...
%Comparaison entre Taux  d’erreur  binaire TEB simulé et théorique pour la chaine_16-QAM
figure
semilogy(vecteur_db_Eb_N0,vecteur_Teb);
xlabel('Rapport  signal à  bruit  Eb/N0 en db');
ylabel('Taux  d’erreur  binaire TEB pour la chaine_16-QAM.');
hold on
...
legend('Taux  d’erreur  binaire TEB simulé pour la chaine_16-QAM ','Taux  d’erreur  binaire TEB théorique pour la chaine_16-QAM.');
title('Comparaison entre Taux  d’erreur  binaire TEB simulé et théorique pour la chaine_16-QAM.');
